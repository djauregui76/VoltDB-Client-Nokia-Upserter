...
public class VoltBulkLoader {
    ...
    //Set customUpsert
   final boolean  blnSetCustomUpsert;
    //CustomColumnList
    String[] columnList;
    ...
    //In Array form for real set
    final VoltType[] m_realcolumnTypes;
    ...
    int m_columnCnt = 0;
    //Map<Integer, VoltType> m_columnTypes;
    TreeMap<Integer, VoltType> m_actualmappedColumnTypes;
    ...
    public VoltBulkLoader(BulkLoaderState vblGlobals, String tableName, int maxBatchSize,String[] columns,
                          BulkLoaderFailureCallBack blfcb) throws Exception {
        this(vblGlobals,tableName,maxBatchSize,false,columns,blfcb);
    }

    public VoltBulkLoader(BulkLoaderState vblGlobals, String tableName, int maxBatchSize, boolean upsertMode,String[] columns,
                          BulkLoaderFailureCallBack failureCallback) throws Exception {
        this(vblGlobals,tableName, maxBatchSize, upsertMode, columns, failureCallback, null);
    }
    public VoltBulkLoader(BulkLoaderState vblGlobals, String tableName, int maxBatchSize, boolean upsertMode,String[] columns,
        ...
        this.blnSetCustomUpsert=upsertMode;
        ...
        m_realcolumnTypes=new VoltType[columnList.length];
        //Build column info so we can build VoltTable
        if (blnSetCustomUpsert) {
            m_colInfo = new VoltTable.ColumnInfo[columnList.length];
        ...
        // Inject here to search for items.
            VoltTable.ColumnInfo ci;
            if (blnSetCustomUpsert) {
                String cname = m_colNames.get(i);

                for(int a=0;a<columnList.length;a++) {
                    if (cname.toLowerCase().equals(columnList[a].toLowerCase())) {
                        VoltType type = m_columnTypes[i];
                        m_realcolumnTypes[a]=m_columnTypes[i];
                        ci = new VoltTable.ColumnInfo(cname, type);
                        m_colInfo[a] = ci;
                    }
                }
              //
...
if(blnSetCustomUpsert) {
    m_columnCnt=columnList.length;

  //  m_actualcolumnCnt = m_mappedColumnTypes.size();
}
