...
public class PerPartitionTable {
    ...
    PerPartitionTable(ClientImpl clientImpl, String tableName, int partitionId, boolean isMP,
        ...
        m_customupsert= firstLoader.blnSetCustomUpsert;
        ...
        m_realcolumnTypes=firstLoader.m_realcolumnTypes;
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                for (int i = 0; i < row_args.length; i++) {
                    final VoltType type;
                    if(m_customupsert){
                      type =m_realcolumnTypes[i];
                    }
                    else{
                          type = m_columnTypes[i];
                    }
                    row_args[i] = ParameterConverter.tryToMakeCompatible(type.classFromType(),
                            currRow.m_rowData[i]);
                }
    ...
    private void load(ProcedureCallback callback, VoltTable toSend) throws Exception {
        if (m_isMP) {
            ...
            if(m_customupsert) {
                int totalrows=toSend.getRowCount();
                String myrow=toSend.toBulkUpsertString(m_tableName.toString());
                m_clientImpl.callProcedure("@AdHoc", myrow);
