...
public final class VoltTable extends VoltTableRow implements JSONString {
    ...
    public String toBulkUpsertString(String tablename) {
        assert(verifyTableInvariants());
        StringBuilder buffer = new StringBuilder();
        StringBuilder bufferrepeatloop=new StringBuilder();

        short colCount = m_buffer.getShort(5);
        assert(colCount == m_colCount);
        bufferrepeatloop.append("UPSERT Into ").append(tablename).append(" (");
        for (int i = 0; i <colCount; i++)
        { if(i==(colCount-1)) {
            bufferrepeatloop.append(getColumnName(i)).append("");
        }else {
            bufferrepeatloop.append(getColumnName(i)).append(",");
        }
        }

        bufferrepeatloop.append(") values (");
        VoltTableRow r = cloneRow();
        r.resetRowPosition();
        while (r.advanceRow()) {
            buffer.append(bufferrepeatloop.toString());
            for (int i = 0; i < m_colCount; i++) {
                switch(getColumnType(i)) {
                    case TINYINT:
                    case SMALLINT:
                    ...
                    case TIMESTAMP:
                        ...
                        if (r.wasNull()) {
                            ...
                            buffer.append("'").append(tstamp).append("'");
                        }
                        break;
                    case STRING:
                        String string = r.getString(i);
     ...
     * @return A string containing a pretty-print formatted representation of this table.
     */
    public String toFormattedString(boolean includeColumnNames,boolean customupsert) {
